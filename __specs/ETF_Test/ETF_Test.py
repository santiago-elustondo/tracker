...
def expected(file, from_path, to_path):
	...
	if par.is_expected:
		...
		oracle = os.path.join('.' + os.sep, os.path.normpath(par.oracle))
...
def actual(file, to_path):
	...
	exec = os.path.join('.' + os.sep, os.path.normpath(par.executable))
...
def find_tests(list, warnings):
	...
	for file in list:
		...
		path = os.path.join(os.path.normpath(par.log_dir), path)
...
def print_results(results):
	...
	"""
	passed = str(sum([item.split(":")[0] == "Success" for item in results]))
